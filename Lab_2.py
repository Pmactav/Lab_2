...
plt.figure(); plt.plot(obsA, marker='o')
plt.axhline(meanA, linestyle='--', label="Mean")
plt.axhline(weightedMean, linestyle='-.', label="Weighted Mean")
plt.axhline(ciLowA, linestyle=':', label="95% CI"); plt.axhline(ciHighA, linestyle=':');
plt.title("Observer A Observations over Time"); plt.xlabel("Observation Index");
plt.ylabel("Value"); plt.legend()
plt.figure(); plt.plot(obsB, marker='o')
plt.axhline(meanB, linestyle='--', label="Mean")
plt.axhline(weightedMean, linestyle='-.', label="Weighted Mean")
plt.axhline(cilowB, linestyle=':', label="95% CI"); plt.axhline(cihighB, linestyle=':');
plt.title("Observer B Observations over Time"); plt.xlabel("Observation Index");
plt.ylabel("Value"); plt.legend();

resMeanA = resA.mean(); resStdA = resA.std(ddof=1)
resSEM_A = resStdA / np.sqrt(len(resA)); margin99 = 2.576 * resSEM_A
plt.figure(); plt.hist(resA, bins='auto', density=True)
...
plt.title("Residual Histogram - Observer A"); plt.legend();

resMeanB = resB.mean(); resStdB = resB.std(ddof=1)
resMeanDevB = resStdB / np.sqrt(len(resB)); margin99 = 2.576 * resMeanDevB
plt.figure(); plt.hist(resB, bins='auto', density=True)
...
plt.title("Residual Histogram - Observer B"); plt.legend(); plt.show();